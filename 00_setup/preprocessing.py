...
# repo location for the data
LOCATIONS = [
    ("../01_source_data/Train", "train_h"),
    ("../01_source_data/Test_Broward", "test_b"),
    ("../01_source_data/Test_Crete", "test_c"),
]  # names of the files we want to use
...
def load_data():
    ...
    for loc, name in LOCATIONS:
        for file in FILES:
            ...
            data[filename] = pd.read_csv(os.path.join(loc, file), compression="gzip")
    return data


# do some basic preprocessing for the calendar data and save it
def calendar_preprocessing(df: pd.DataFrame) -> pd.DataFrame:
    ...
    data[["price", "adjusted_price"]] = data[["price", "adjusted_price"]].astype(float)
    data["month"] = data.date.dt.month
    price_mean_std = (
        data.groupby(["listing_id", "month"], as_index=False)[["adjusted_price"]]
        .agg(["mean", "std"])
        .reset_index()
    )
...
# separate bathrooms into 4 categories
def bathroom_cat(df: pd.DataFrame) -> np.ndarray:
    ...
    bathroom_choices = [1, 2, 3, 4]
    return np.select(bathroom_condition, bathroom_choices, 5)


# separate bedrooms into 5 categories
...
# separate number of beds into 5 categories
...
# create a categorical variable for all the reviews
def ratings_columns(train_df: pd.DataFrame) -> pd.DataFrame:
    ...
    rating_quantiles = (
        train_df[ratings].describe().filter(["25%", "50%", "75%"], axis=0)
    )
...
def to_digits(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["host_response_rate"] = (
        df["host_response_rate"].str.split("%").str[0].astype(float)
    )

    df["host_acceptance_rate"] = (
        df["host_acceptance_rate"].str.split("%").str[0].astype(float)
    )
...
def cat_to_num(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["host_response_time"] = df.host_response_time.map(host_response_time_map).fillna(
        5
    )

    df["room_type"] = df.room_type.map(room_type_map).fillna(5)
    return df


# calculate the length of certain text fields
def len_based_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["host_verifications"] = df.host_verifications.apply(lambda x: eval(x))

    df["host_verifications_len"] = df.host_verifications.apply(lambda x: impute_len(x))
    return df


# find key words in the amenities text field
def amenities_one_hot(df: pd.DataFrame) -> pd.DataFrame:
    ...
    :return:
    """
    df["amenities_ls"] = df["amenities"].apply(lambda x: eval(x.lower()))
    df["has_wifi"] = df["amenities_ls"].apply(lambda x: 1 if "wifi" in x else 0)
    df["has_pool"] = df["amenities_ls"].apply(lambda x: 1 if "pool" in x else 0)
    df["has_kitchen"] = df["amenities_ls"].apply(lambda x: 1 if "kitchen" in x else 0)
    df["has_washer"] = df["amenities_ls"].apply(lambda x: 1 if "washer" in x else 0)
    df["has_dryer"] = df["amenities_ls"].apply(lambda x: 1 if "dryer" in x else 0)
    df["has_ac"] = df["amenities_ls"].apply(
        lambda x: 1 if "air conditioning" in x else 0
    )
    df["has_self_checkin"] = df["amenities_ls"].apply(
        lambda x: 1 if "self check-in" in x else 0
    )
    df["has_workspace"] = df["amenities_ls"].apply(
        lambda x: 1 if "laptop-friendly workspace" in x else 0
    )
    df["has_pet_allowed"] = df["amenities_ls"].apply(
        lambda x: 1 if "pets allowed" in x else 0
    )
    df["has_free_parking"] = df["amenities_ls"].apply(
        lambda x: 1 if bool(filter(lambda v: match(r"free(.*)parking", v), x)) else 0
    )
    return df


# use tfidf to find the most important words in the description and neighborhood text fields
def text_to_features(df: pd.DataFrame, train: bool) -> pd.DataFrame:
    ...
    if train:
        ...
        neighborhood = tf_idf_n.fit_transform(
            df.neighborhood_overview.fillna("").to_list()
        )
...
# impute missing values as zero in the appropriate columns
def simple_impute(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["neighborhood_overview_len"] = df["neighborhood_overview_len"].fillna(0)

    df["reviews_per_month"] = df["reviews_per_month"].fillna(0)
    return df


# categorize rating percentiles
...
# get only the columns that are needed for the model
...
# create the full dataset for the price prediction model
def price_prediction_dataset(listings_df: pd.DataFrame, calendar_df) -> pd.DataFrame:
    """
    ...
    merged_df = pd.merge(
        l_df,
        calendar_df[["listing_id", "month", "log_price_mean", "log_price_std"]],
        left_on="id",
        right_on="listing_id",
    )
...
# create the full dataset for the availability prediction model
def availability_prediction_dataset(
    listings_df: pd.DataFrame, average_price_df: pd.DataFrame
) -> pd.DataFrame:
    """
    ...
    """
    merged_df = pd.merge(
        listings_df, average_price_df, left_on="id", right_on="listing_id"
    )
    merged_df.drop("listing_id", axis=1, inplace=True)
    target_conditions = [
...
if __name__ == "__main__":
    ...
    dfs = [
        ...
        ("test_c_listings", "test_c_calendar", False, "crete_reg.csv", "crete_cat.csv"),
    ]
